...
*/
        ...
        System.out.println("testDataDataSet.numExamples():"+testDataDataSet.numExamples());

        System.out.println("testDataDataSet.getFeatures().data().length():"+testDataDataSet.getFeatures().shape()[2]);
        INDArray predictionsTestdata  = net.rnnTimeStep(testDataDataSet.getFeatures());

        System.out.println("predictionsTestdata.data().length():"+predictionsTestdata.shape()[2]);

     //   INDArray testDataInput = testDataDataSet.getFeatures().get(NDArrayIndex.interval(19,20), NDArrayIndex.all());
       // INDArray testDataInput = testDataDataSet.getFeatures().get(NDArrayIndex.interval(1,3,true));

       // log.info("raw testDataInput:"+testDataInput);
...
/*
        ...
        log.info("revert testDataInput:"+testDataInput);

        int rows = (int)testDataInput.shape()[2];
        log.info("revert testDataInput Number of rows:"+rows);
        for (int i = 0; i < rows; i++) {
            log.info("testDataInput({}): {}:",i,testDataInput.getDouble(i));
        }
*/
...
/*
        double[] lastInputSequence = {timeSeries[timeSeries.length - timeSteps]};
        INDArray forecastInput = Nd4j.create(lastInputSequence);
        INDArray forecastOutput = model.rnnTimeStep(forecastInput);
*/
       ...
       // createSeries(c, predictedOutput, trainSize , "Predicted output data");
