 ...
 * <ul>
 *   <li>Gap-down &gt; 0.8% (today's open vs. previous day's close)</li>
 *   <li>Gap-down &lt; 3.0% (larger gaps often signal real news — don't fade those)</li>
 *   <li>After bar 1 (wait 15 min for dust to settle)</li>
 *   <li>No new day-low on current bar (support is holding = "morning low holds")</li>
 *   <li>No entry after bar 8 (~11:00, gap fills should happen in the morning)</li>
 * </ul>
 *
 * <h3>Exit rules (first satisfied wins)</h3>
 * <ul>
 *   <li>Profit target: price reaches previous day's close (gap fill complete)</li>
 *   <li>Stop: 0.4% below entry (tight stop — if the gap doesn't fill, cut fast)</li>
 *   <li>Max 20 bars held (~5 hours) — close by end of day</li>
 * </ul>
 *
 * <h3>Risk/reward design</h3>
 * <p>Risk: 0.4% stop. Reward: 0.8–3.0% gap fill. Typical R:R is 2:1 to 5:1.
 * Even at 55% win rate this is highly positive expectancy.
 */
@Component
@Slf4j
public class GapReversalIntradayStrategy extends AbstractStrategy implements IIndicatorValue, IntradayStrategy {
    ...
    // ── Parameters ─────────────────────────────────────────────────────────
    private static final double GAP_MIN_PCT       = -3.0; // Max gap-down (more negative = larger gap)
    private static final double GAP_MAX_PCT       = -0.8; // Min gap-down to trigger
    private static final int    MIN_BAR_FOR_ENTRY = 1;    // Wait 1 bar (15 min) after open
    private static final int    MAX_BAR_FOR_ENTRY = 8;    // No entries after ~11:00
    private static final double STOP_LOSS_PCT     = 0.4;  // Tight stop
    ...
    public Strategy buildStrategy(BarSeries series) {
        ...
        Rule entryRule = gapDown.and(afterFirstBar).and(notTooLate).and(supportHolds);
